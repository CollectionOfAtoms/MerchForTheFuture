...
describe("US-MFTF-17.8 — placement tool", () => {
  it("opens centred at the 90% tool default when there is no saved placement", () => {
    renderPanel();
    expect(nums()).toMatchObject({ x: 0.5, y: 0.5, scale: 0.9, angle: 0 });
  });
  ...
  it("the resize handle scales, clamped to the max", () => {
    ...
    fireEvent.pointerUp(surface, { pointerId: 1 });
    expect(nums().scale).toBeCloseTo(1.4); // starts at the 0.9 tool default
  ...
  it("'Reset to centered' clears the saved placement", async () => {
    ...
    // …and the tool returns to the centred 90% starting default locally.
    await vi.waitFor(() => expect(nums()).toMatchObject({ x: 0.5, y: 0.5, scale: 0.9, angle: 0 }));
  });
