...
| Teemill per-product editor deep-link URL | Open (live-confirm) | The referenced-listing edit page (US-MFTF-13.4) offers an "Edit on Teemill" link. The catalog exposes `ref` and `slug`, but a stable per-product **editor** URL (and whether it needs the founder's authenticated Teemill session) is unverified. Ship a dashboard/generic-designer fallback link until the pattern is confirmed live. |
