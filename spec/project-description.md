...
| Printify / Printful | Second/third apparel dropshippers. Printify (Epic MFTF-17) is in active implementation as of 2026-07-11; Printful (Epic MFTF-18) is scoped identically but sequenced after it | Material-standard gate cleared 2026-07-11 for both — qualifying styles confirmed for each. Sourcing mode (`DESIGNED` vs `REFERENCED`) is unresolved for both and is the subject of a per-provider API discovery spike (US-MFTF-17.1 / US-MFTF-18.1, documented in `/docs/printify-api-notes.md` / `/docs/printful-api-notes.md`) before implementation is scoped in detail |
| MailerSend | Free tier available at current scale | Current transactional email provider. Intercepted in tests via MSW at https://api.mailersend.com/v1/email — do not make real API calls in tests. A vestigial **Resend** mailer is retained as a fallback in case of revert; some older spec stories (e.g. US-22.4) still name Resend — read those as "the transactional mailer." |
...
| Which dropshippers will be integrated beyond Prodigi? | Partially resolved | Teemill integrated (REFERENCED). Printify (Epic MFTF-17) material-standard gate cleared 2026-07-11 and is in active implementation; sourcing mode (DESIGNED vs REFERENCED) pending its API discovery spike (US-MFTF-17.1). Printful (Epic MFTF-18) gate also cleared, scoped identically, sequenced after Printify. |
| Printify / Printful sourcing mode (DESIGNED vs REFERENCED) | Partially resolved | **Printify (proposed 2026-08-18, pending both-founder ratification): DUAL-MODE.** The US-MFTF-17.1 spike chose DESIGNED and US-MFTF-17.2/17.7–17.9 built it; a later realisation is that Printify's order API *also* accepts a REFERENCED form (`{ product_id, variant_id }`) pointing at a product built in **our own** Printify shop — the Teemill (MFTF-13) pattern with the shop being ours. Since `SourcingMode` is already per-listing, Printify becomes the first provider offered in **either** mode, chosen per listing: DESIGNED = cheap breadth (drop a design on any blank), REFERENCED = curated fidelity (real per-colour Printify mockups). Both kept (nothing is live, so a clean addition). Scoped as US-MFTF-17.12–17.14, reusing MFTF-13 wholesale — see the epic file. **Printful (Epic MFTF-18) remains Open** — its own spike (US-MFTF-18.1) decides independently; do not assume it matches Printify. |
...
| Per-seller Printify accounts / multi-store (external sellers bring their own API key) | Deferred | Surfaced 2026-08-18 alongside the dual-mode Printify decision. Today there is one env-level `PRINTIFY_API_KEY` + `PRINTIFY_SHOP_ID` (single founder account). If non-founder sellers are ever onboarded, one clean model is: each seller connects their **own** Printify account (their key + shop), builds their own products, and the platform **references** them — which fits the REFERENCED lane cleanly (per-seller product ownership) and the single-shared-shop DESIGNED lane poorly. Implementation would move provider credentials to encrypted per-seller storage and resolve the key/shop per listing's seller at ingest/order time. **May never be built** — it is gated on actually having multiple sellers (see the row above), which there is no concrete plan for. Logged so the dual-mode design keeps this door open, not as scheduled work. |
...
| Ongoing SEO content strategy (blog/journal) | Deferred | Epic MFTF-22 (2026-07-11) covers the technical SEO foundation only (sitemap, metadata, structured data) — a one-time-setup layer. Ongoing content (a blog/journal for ranking on fresh content over time) was explicitly scoped out as a separate, larger commitment requiring an ongoing content cadence, not just code. Revisit as its own epic if wanted. |
...
| Seller-controlled print framing, canvas wrap & buyer mockups | Resolved | Resolved 2026-06-21 (Epic MFTF-PF). Per-aspect framing crop (canvas **and** paper) sent to Prodigi as the production asset; canvas edge-wrap is seller-fixed (default `MirrorWrap`; `ImageWrap` excluded from the picker — enum keeps all four values, UI/server-guarded); buyer display uses seller-uploaded per-size mockups (`PrintSizeMockup`), not Prodigi-generated previews. Hard gate blocks `ACTIVE` until every offered aspect is framed **and** every offered size has a mockup; strict archive-on-deploy for non-compliant live listings. Replaces the hardcoded `sizing:"fillPrintArea"`/no-wrap path in `prodigi.ts`. Revises Passed US-15.2/15.4/15.6/15.7. Decisions A–E + gate + backfill founder-ratified 2026-06-21. |
| Prodigi-mockup display for prints (resolves the deferred mockup row, prints side) | Resolved (prints) | Epic MFTF-PF settles this for fine-art prints: buyer print display is **seller-uploaded** per-size mockups, not Prodigi-generated. The deferred "Display/ingest of Prodigi's own mockups" row above remains open for **designed apparel** listings only. |
| project_description.md "seller never sets up a print" phrasing | Deferred (clarified) | The Cart & Checkout Model line was clarified inline (Epic MFTF-PF, 2026-06-21): prints still aren't separate listings, but sellers now configure framing/wrap/mockups. A fuller editorial pass to scrub any remaining "seller never sets up a print" language across the doc is a low-priority Mode-F cleanup. |
| Teemill US-landed cost — automated quoting | Deferred | Scoped 2026-06-25 (Epic MFTF-19). The MVP (US-MFTF-19.5/19.6) captures US-landed cost as a **founder-entered** USD value and displays it (number + admin-set color band) on the admin catalog view and seller picker. Automating the number via a live `POST /orders` quote to a US address is deferred: it is gated by Teemill Open Q#3 (undocumented rate limits) and the still-`UNVERIFIED` question of whether an unconfirmed `POST /orders` expires/bills, and is complicated by the £0 bundled-shipping quirk (Teemill folds shipping into item cost). The field is shaped so an automated quoter can later populate the same column with no migration. Revisit post-launch once a real US order reveals the quote shape. |
| Mandatory apparel categorization | Resolved | Resolved 2026-08-17 (Epic MFTF-24, US-MFTF-24.5). A listing cannot go `ACTIVE` without at least one **durable grouping** — a `MANUAL` category (Men's/Women's) or a `MANUAL` collection membership. Drafts save freely, matching the Epic MFTF-PF publish-gate precedent. **Whatevs and New deliberately do not satisfy the gate:** Whatevs contains every listing so it expresses no choice, and `New` is recency-derived, meaning it is true of every listing at publish (a gate on it could never fail) and stops being true roughly one window later — exactly when the listing would become unreachable. An earlier pass in the same session had made membership optional; this supersedes it. The one residual gap — a live listing losing its last grouping after publication — is covered advisorily by US-MFTF-24.3's last-grouping warning; a standing "uncategorized listings" badge/count was considered and deferred as unnecessary at current catalog size. |
| `/shop` vs. the Whatevs category page | Resolved (deliberate duplication) | Resolved 2026-08-17 (Epic MFTF-24). Whatevs is defined as *all apparel*, and the founder chose to give it its own route (`/shop/category/whatevs`) rather than aliasing `/shop`. The two therefore render the same result set today. Recorded explicitly so a future session doesn't "fix" the duplication as an oversight: the separation is what allows `/shop` to later become something else (a landing page, a mixed-media grid) without disturbing the category system. |
| Nested / hierarchical apparel categories | Deferred | Scoped 2026-08-17 (Epic MFTF-24). Categories are a flat set (Men's, Women's, Whatevs). Sub-categories (e.g. Men's → Tees) would need a self-referential parent on `Category` plus a deeper breadcrumb trail. Not warranted at a ~10-design catalog size; revisit if the catalog grows enough that a flat list stops being navigable. |
| Categories/collections for fine-art prints & originals | Deferred | Scoped 2026-08-17 (Epic MFTF-24), which is apparel-only. The `/browse` fine-art surface keeps its existing Epic 7 filtering. Whether prints and originals get the same category/collection treatment — and whether the two catalogs should ever share one taxonomy — is unscoped, and interacts with the long-deferred "unified browse page" question above. |
| Apparel design/placement tool back-printing | Deferred | Scoped 2026-08-16 alongside US-MFTF-17.7–17.11 (front-only in that scope). `ApparelListing` has exactly one design file and the current placement model is front-only; back printing needs a second design-file upload slot and doubled placement UI/schema. Not built until a real seller need surfaces. |
| Per-variant apparel placement precision | Deferred | Scoped 2026-08-16 alongside US-MFTF-17.7–17.11. The placement tool captures one representative (modal) print-area dimension per product type and applies one placement across every offered colour/size, accepting minor per-variant print-area pixel-size drift rather than requiring the seller to place their design once per (colour,size) combo. Revisit only if that drift proves visually significant in practice; the schema (unique-per-listing today) can extend to per-variant rows without a structural rewrite. |
| Prodigi apparel design/placement parity | Open | Prodigi apparel has the identical auto-fill-to-centre gap the Printify placement tool (US-MFTF-17.7–17.11, scoped 2026-08-16) fixes for Printify. Not scoped there, per the New-Provider Pattern's Printify-first sequencing — Prodigi parity would be its own follow-up story once the Printify tool has shipped and its patterns are proven. |
...
|---|---|---|
| 2026-08-25 | **Both founders RATIFIED the 2026-08-18 Printify dual-mode proposal** and implementation began. Confirmed direction: Printify supports **DESIGNED and REFERENCED** listings to leverage both lanes (breadth vs. curated fidelity), promoting US-MFTF-17.12/17.13/17.14 from PROPOSED to ratified/schedulable. Implementation started with **US-MFTF-17.12** (REFERENCED ingest — cache a Printify shop product by `product_id` into the existing MFTF-13 referenced schema additively, no migration) via TDD. Also this session: US-18.4 promoted to `Passed` after live focal-point confirmation and archived (see PR #23). No spec content changed beyond the ratification note in `epics/mftf-17-printify-integration.md` and this entry. | MFTF-17 |
| 2026-08-20 | Implementation session (design-system adoption branch): added **US-18.4 (Seller-Specified Focal Point for the Browse-Grid Crop)** to Epic 18 and implemented it. Context: this branch replaced the artwork browse/prints **masonry** (`columns-*`, variable-height tiles) with a fixed-**square** grid (`grid-cols`, `object-cover`), which supersedes US-18.2's "masonry browse tiles" presentation for that surface — a square crop necessarily cuts a non-square piece, so sellers needed control over what survives the crop. Design decision (founder-directed): a **focal point** (`ArtworkImage.focalX`/`focalY`, `Float @default(0.5)`, additive migration) applied as CSS `object-position` on the tile, chosen over a full square crop-rect + generated `gridUrl` — no image processing, tiny schema surface, instant. The `PrintFramingPanel` crop-rect pattern (US-MFTF-PF) was considered but is heavier than warranted for a display-only focal point. Seller sets it on the listing **edit** page via a marker over the primary image with a live square-crop preview; the browse projection (`ArtworkCard`) carries the focal point and `ListingCard` renders `object-position`; null/legacy → centre. Tracker: US-18.4 added `Not Started` (→ implemented same session). | Epic 18 |
| 2026-08-18 | Spec session (**PROPOSED, pending both-founder ratification** — reverses/extends the US-MFTF-17.1 DESIGNED-only conclusion, so unsettled until both founders sign off): scoped **Printify as DUAL-MODE** — keep the DESIGNED lane (US-MFTF-17.7–17.9, built + passing) and add a REFERENCED lane as three new stories **US-MFTF-17.12** (ingest a Printify product built in our own shop by `product_id`, into the existing MFTF-13 referenced schema additively), **US-MFTF-17.13** (a "new referenced Printify listing" flow + a Printify-only DESIGNED-vs-REFERENCED creation fork, mirroring US-MFTF-13.3), and **US-MFTF-17.14** (order by `{product_id, variant_id}` + re-sync per US-MFTF-13.4). The realisation driving it: Printify's order API accepts **both** an ad-hoc DESIGNED form (`print_areas`, the design on the order) and a REFERENCED form (`product_id`, pointing at a product already built in our shop) — the 17.1 spike correctly ruled out an *external* reference model (à la Teemill) but under-weighted that a product in our **own** shop is referenceable, functionally the MFTF-13 pattern with the shop being ours. Since `SourcingMode` is per-listing, Printify becomes the first provider offered in either mode. Rationale for keeping both (not replacing DESIGNED): the lanes serve distinct jobs — DESIGNED = cheap breadth (a design onto any curated blank), REFERENCED = curated fidelity (real per-colour Printify mockups, colour-accurate garments) at the cost of the founder authoring each product in Printify's dashboard. Founder-driven decisions (pending ratification): keep 17.7–17.9; nothing is live so it's a clean addition (no migration); reuse MFTF-13 almost wholesale (this also realises the "if REFERENCED" criteria parked in US-MFTF-17.2); REFERENCED ingest and the deferred DESIGNED save-time preview (17.10/17.11) share one primitive ("create/read a Printify product + its mockups"), so 17.11 should reuse REFERENCED's machinery; sequence by real launch-imagery need (left `Not Started`, not urgent — one seller, nothing live); the material-standard gate stays manual founder curation. Added a deferred Open Question for **per-seller Printify accounts/keys** (external sellers bring their own key + shop — fits REFERENCED cleanly; gated on ever having multiple sellers; may never be built). Tracker: US-MFTF-17.12/17.13/17.14 added `Not Started`. No code, schema, or tests touched — scoping only. | MFTF-17 |
| 2026-08-17 | Spec session: added **Epic MFTF-24 (Apparel Categories, Collections & Navigation)** — 7 stories. A merchandising layer over the apparel catalog, which until now had exactly one undifferentiated surface (`/shop`, every ACTIVE listing newest-first). Adds: buyer-facing categories Men's/Women's/Whatevs (24.1 schema, 24.2 pages, 24.5 seller assignment on both designed and referenced listing forms), admin-managed collections seeded with New + Our Favorites (24.1, 24.3 admin CRUD, 24.4 public pages + index), a path-aware product breadcrumb (24.6), and an Apparel nav dropdown for desktop and mobile (24.7). Founder-ratified decisions: (1) category membership is **non-exclusive but mandatory to go `ACTIVE`** — a listing may be in both, one, or (as a draft) neither, but cannot publish without a **durable grouping**: a `MANUAL` category or `MANUAL` collection membership. Settled after two passes in the same session — an optional-membership pass first, then this gate. Notably, the founder's initial phrasing ("even if it just needs to fall into New") was found **unimplementable as stated**: `New` is `AUTO_NEW`/recency-derived, so it is true of every listing at publish — a gate on it could never fail — and ceases to be true roughly one window later, which is precisely when the listing becomes unreachable. The gate therefore excludes both `New` (decays) and Whatevs (contains everything, so expresses no choice), and a test pins the Whatevs-and-New-only case as rejected so it cannot regress. Existing `ACTIVE` listings are logged rather than auto-archived — a deliberate divergence from Epic MFTF-PF's strict archive-on-deploy, on the grounds that an unframed print is a defective product whereas an uncategorized listing merely sells while being harder to find; (2) **Whatevs = all apparel on its own route**, deliberately duplicating `/shop`'s result set (also logged, so it isn't later "fixed" as an oversight); (3) **New is automatic with admin pin/exclude overrides** (recency window ∪ pins − excludes), which is why `Collection` carries a `kind` instead of every collection being a plain join table; (4) breadcrumb default trail `Home → Whatevs → first collection → listing title`, with "first" made deterministic by `Collection.sortOrder` + slug tie-break. Two cross-epic findings recorded rather than left to collide: **US-MFTF-24.6 revises US-MFTF-22.3** (whose fixed `Home → Shop → [Product]` BreadcrumbList is superseded — the JSON-LD now emits the *default* trail, never the visitor-specific one, since it must describe the canonical URL), and MFTF-24's four new public routes require sitemap + metadata coverage in US-MFTF-22.1/22.2 with `?from=` excluded from canonicals. epicOrder: **MFTF-24 inserted before MFTF-22** for exactly that reason — doing SEO first would mean writing the breadcrumb and route metadata twice. Also noted: `/shop/category/[slug]` route depth is load-bearing, since a flatter `/shop/mens` would collide with the existing `/shop/[listingId]` dynamic segment. No code, schema, or tests touched — scoping only. | MFTF-24, MFTF-22 |
| 2026-08-16 | Spec session: scoped the EMERGING **US-MFTF-17.7 Seller Apparel Design/Placement Tool** placeholder (raised by the founder 2026-08-15) into five real stories — US-MFTF-17.7 (capture Printify print-area placeholders at sync), US-MFTF-17.8 (seller placement tool: drag/scale/rotate, reusing the `FramingTool`/`crop-geometry` pointer-drag pattern), US-MFTF-17.9 (order-time positioned `print_areas` wiring), US-MFTF-17.10 (a chore-tracked spike prototyping Printify's mockup call, mirroring US-MFTF-17.1's spike/implementation shape), and US-MFTF-17.11 (the resulting confirmation preview, scoped at the outcome level pending 17.10's finding). Key decisions: (1) full freeform drag+scale+rotate — parity with Printify's `x/y/scale/angle` — over simpler centre/fill/fit presets; (2) front-only in this scope, back-printing deferred (see Open Questions); (3) the live-editing canvas needs an immediate in-browser composite regardless (ships in 17.8), but the higher-fidelity save-time preview (own composite vs. Printify draft-product mockups) is deferred to a spike (17.10) + follow-up (17.11) rather than decided up front; (4) one placement per listing (not per-variant), accepting minor per-variant print-area dimension drift (see Open Questions); (5) Printify-first per the New-Provider Pattern — Prodigi apparel has the identical gap but is intentionally not touched here (see Open Questions). Tracker: US-MFTF-17.7 flipped `Deferred`→`Not Started` and retitled; US-MFTF-17.8/17.9/17.11 added `Not Started` (US-MFTF-17.10 is a chore, no tracker row, matching US-MFTF-17.1's precedent). No code, schema, or tests touched — scoping only. | MFTF-17 |
| 2026-07-11 | Confirmed the `tdd-handoff`/`tdd-spec-session` skills are **not** written to work with the 2026-07-11 tracker/spec split — their kickoff instructions ("upload your three source-of-truth files," "read all three completely," the Handoff Prompt Template's "I've attached three files... read all three completely") assume one spec file and one complete tracker, and `tdd-handoff`'s documented Tracker Schema (nested `epics: [{stories}]`, lowercase `status` values) doesn't match this project's flat-array, capitalized-status schema either (pre-existing mismatch, not new). Cannot edit the skill files (read-only plugin cache). Mitigation: added an explicit tripwire to both `spec/README.md` and `AGENTS.md` instructing any session about to run either skill to stop and substitute `spec/README.md`'s file list for the skill's own kickoff file requests. No skill files were or can be modified — this is a project-side workaround, not a skill fix. | — |
| 2026-07-11 | Added **Epic MFTF-23 (Admin Tracker Archive Merge)** — 1 story — a regression fix from the same-day tracker split: `/admin/tracker` (US-MFTF-19.2) reads only `spec/project-tracker.json`, which post-split holds just the 33 active stories, so the page's completion counts and per-epic sections silently went wrong for every historically-Passed epic. US-MFTF-23.1 merges both `project-tracker.json` and `project-tracker-archive.json` before grouping/counting, with a visible fallback warning if the archive file is unavailable. Founder priority: sequenced **first** in `epicOrder.sequence`, ahead of MFTF-17 (Printify), since it should land before further tracker-dependent work continues. | MFTF-23 |
| 2026-07-11 | **Structural change (not content):** split the spec and tracker files, which had grown to ~140K tokens combined (`user-stories-art-marketplace.md` was 254KB/45 epics inline; `project-tracker.json` was 257KB/210 stories, 174 already Passed). (1) Tracker split into `project-tracker.json` (active: Not Started/Deferred/pending-live-confirmation, 33 stories, ~42KB) and `project-tracker-archive.json` (Passed/Dropped, 177 stories, ~217KB) — active file kept its original name so existing tooling still finds it. (2) Spec split into `spec/epics/*.md`, one file per epic (45 files, ~1–18KB each); `user-stories-art-marketplace.md` became a thin index (epic list + status + link into `epics/`), ~8KB. Both splits verified programmatically as lossless — all 173 story headers and all 429 acceptance-criteria checkboxes accounted for exactly once, zero duplicates, zero omissions; no content was rewritten, only relocated. New protocol documented in `spec/README.md` and pointed to from `AGENTS.md`, since the generic `tdd-handoff`/`tdd-spec-session` skill templates assume a single-file spec and single-file tracker and needed a project-specific override so future sessions read only the relevant epic file plus the active tracker, not everything. No epic/story content, status, or numbering changed in this entry. | — |
| 2026-07-11 | Spec session: added **Epic MFTF-22 (SEO Foundation)** — 3 stories, plus **US-MFTF-10.9** appended to the Pre-Launch Checklist. Audited current metadata state: only the two detail pages (`/shop/[listingId]`, `/artwork/[id]`) have `generateMetadata()`; nothing else exists (no sitemap, robots.txt, canonical URLs, OG/Twitter completeness, or structured data anywhere). Scoped as the "full technical foundation" pass per founder decision — sitemap.xml + robots.txt (22.1), complete metadata across all public pages including the new About/Contact pages (22.2), and Product/Organization/BreadcrumbList JSON-LD (22.3, sourced from the same `FounderBio` data as US-MFTF-20.2 — flagged as blocked-until-20.2-ships). Ongoing content strategy (blog/journal) was explicitly scoped out as a separate, larger commitment — see Open Questions. US-MFTF-10.9 (Search Console verification + sitemap submission) added as a manual founder task, sequenced after MFTF-22 and after go-live (10.8). epicOrder: MFTF-22 inserted after MFTF-21, before MFTF-10. | MFTF-22, MFTF-10 |
| 2026-07-11 | Spec session: added **Epic MFTF-20 (About & Contact Pages)** — 2 stories — and **Epic MFTF-21 (Contact Page & Feedback Form)** — 2 stories. MFTF-20: `/about` page with three scroll sections (vision & goals, Who We Are founder bios for Elle Sparks then Jesse Caldwell with oval photo frames, material-standard commitment); scope decision made this admin-editable (small CMS: `AboutPageContent` + `FounderBio` models, rich-text editor, reuses the existing Blob image pipeline for photos) rather than static copy, after the founder confirmed all three sections should be editable. Elle's bio links to ElleSparksCreations.com. MFTF-21: `/contact` page with a form emailing ThePeople@MerchForTheFuture via MailerSend, thank-you confirmation, honeypot spam protection, and per-IP rate limiting (soft-limit caveat if implemented in-memory in a serverless context). Both epics are independent of the fulfillment critical path; epicOrder: inserted after Infrastructure, before Epic MFTF-10 (Pre-Launch Checklist) — a public-facing site should have both before go-live. | MFTF-20, MFTF-21 |
| 2026-07-11 | Reordered epicOrder.sequence: moved **Epic MFTF-10 (Pre-Launch Checklist)** to the very end of the sequence, after Infrastructure, per founder decision that go-live should be the final gate following all other work including open infrastructure chores (Vercel Pro, DNS, live credentials) rather than preceding them. | MFTF-10 |
| 2026-07-11 | Spec session: cleared the material-standard gate for **Epic MFTF-17 (Printify)** and **Epic MFTF-18 (Printful)** — qualifying styles confirmed for both, meeting the sustainably-sourced + biodegradable standard. Expanded both epics from one-line deferred stubs into the three-phase pattern used for Teemill (discovery spike → provider integration → founder live-order confirmation): US-MFTF-17.1/18.1 (API discovery spikes, output `/docs/printify-api-notes.md` and `/docs/printful-api-notes.md`), US-MFTF-17.2/18.2 (provider integration, acceptance criteria split mode-independent vs. DESIGNED-specific vs. REFERENCED-specific pending each spike's finding), US-MFTF-17.3/18.3 (founder-executed live order, gates Passed status). Printify is in active implementation now; Printful is scoped identically but sequenced after it and deferred in the tracker pending founder bandwidth. Sourcing mode (DESIGNED vs REFERENCED) is an open question for both providers, added to Open Questions — resolved independently per provider, not assumed to match Teemill or each other. epicOrder: MFTF-17 inserted into the active sequence after Epic MFTF-16, before MFTF-PF. | MFTF-17, MFTF-18 |
| 2026-06-25 | Spec session: added **Epic MFTF-19 (Storefront Polish & Pricing Visibility)** — 7 stories. (1) Apparel carousel now shows the lifestyle-then-mockups **union** instead of lifestyle replacing mockups (revises Passed US-MFTF-6.2). (2) Navigable admin tracker page with section nav + per-story passed-dates. (3/4) OS-aware, cookie-persisted **dark mode** (tokens story + no-flash SSR toggle). (5/6) **Teemill US-landed cost**: founder-entered USD value (manual MVP, automation deferred — see Open Questions) shown with number + admin-set color band on both the admin catalog view and the seller picker. (7) Per-mockup **background color** for transparent Teemill mockups — stored as an open-ended color value (not an enum), composited at render time, image never modified. epicOrder: MFTF-19 inserted after Epic 5 (Tax), before MFTF-10. Also reconciled the spec file with the tracker: added the four tracker-only Passed stories missing from the spec (US-6.4 Email Verification, US-14.7 Runner-Up Offer, US-14.8 Fulfillment Error Email, US-23.2 Restructured Mobile Menu) and annotated the MailerSend/Resend provider note (MailerSend current, Resend retained as fallback). | MFTF-19 |
...
| 2026-06-21 | Spec session: added Epic MFTF-PF (Print Framing, Canvas Wrap & Seller Mockups) — 7 stories. Seller-controlled per-aspect framing crop (canvas + paper) → Prodigi production asset; canvas edge-wrap seller-fixed (default `MirrorWrap`, `ImageWrap` excluded from picker, enum retains all four values); buyer display via seller-uploaded per-size mockups (`PrintSizeMockup`). Hard publish/activation gate (every offered aspect framed + every offered size mocked); strict archive-on-deploy backfill for non-compliant live listings. Replaces hardcoded `sizing:"fillPrintArea"`/no-wrap in `prodigi.ts`. Clarified the Cart & Checkout "prints do not become listings" note (sellers now configure framing/wrap/mockups, but prints still aren't separate listings). Revised Passed US-15.4 (buyer preview source → seller-uploaded) and US-15.7 (canvas cost-calc wrap → `MirrorWrap` default, was "White"); US-15.2 extended, US-15.6 unchanged guardrail. Decisions A–E + gate strictness + buyer-mockup model founder-ratified. epicOrder: MFTF-PF inserted after MFTF-16, before Epic 5. | MFTF-PF |
| 2026-06-22 | Implementation session: completed Epic 5 (Tax Calculation via **Stripe Tax**, not TaxJar). US-5.1 enables `automatic_tax` across both checkout flows behind `STRIPE_TAX_ENABLED` (default off pre-launch) with per-line `tax_behavior`/`tax_code`, persists Stripe-computed tax to the Order, and adds `docs/tax-configuration.md`. US-5.2 adds buyer exemption certificates (admin-approved) that set the buyer's Stripe Customer `tax_exempt`; checkout attaches the Customer. US-5.3 adds an admin `/admin/tax` page linking to Stripe Dashboard tax reports + a nexus panel (Connect/1099-K confirmed moot — single-seller). US-5.4 adds display-only multi-currency (daily FX cron + `ExchangeRate` cache; account/cookie/geo currency resolution; USD settlement unchanged). Removed the leftover TaxJar `calculate.ts`/`reporting.ts` scaffolding and its MSW handler. | Epic 5 |
...
> _**Clarification (Epic MFTF-PF, 2026-06-21):** a print still does **not** become its own listing — buyers continue to pick size/material at purchase time off the artwork. But the seller **does** now have a print-configuration surface on the listing: per-aspect framing crops, canvas edge-wrap, and per-size buyer mockups (US-MFTF-PF). Earlier phrasing elsewhere that "the seller never sets up a print" predates US-15.2 and is superseded — the seller sets up **how** a print is framed/displayed, not a separate print listing. See Open Questions._
