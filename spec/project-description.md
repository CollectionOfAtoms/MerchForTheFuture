...
| Printify / Printful sourcing mode (DESIGNED vs REFERENCED) | Open | Unlike Teemill, not obvious up front — each provider's API discovery spike (US-MFTF-17.1 / US-MFTF-18.1) must determine whether the seller uploads a design onto a curated blank or the founder builds the product on the provider's own site and pastes a reference. Findings land in `/docs/printify-api-notes.md` / `/docs/printful-api-notes.md`. Do not assume Printful matches whatever Printify resolves to. |
